...
import AttributeGrid from "./(AttributeAction)/_AttributeGrid";
import CreateAttribute from "./(AttributeAction)/_CreateAttribute";
import EditAttribute from "./(AttributeAction)/_EditAttribute";
import "./style.scss";
...
export default function VendoAttributer() {
  ...
  useEffect(() => {
    if (setSelectedMasterDatatab) {
      setSelectedMasterDatatab("Attribute");
    }
  }, [setSelectedMasterDatatab]);
  const auth = UseAuth();
  if (!auth) {
  ...
  const tabRenderValuePlant: Record<string, ReactNode> = {
    ...
    edit: <EditAttribute EditDataGet={EditDataGet} />,
  };
