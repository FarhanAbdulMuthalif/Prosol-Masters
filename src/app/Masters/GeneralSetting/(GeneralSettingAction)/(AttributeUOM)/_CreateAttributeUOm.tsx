...
export default function CreateAttributeUOM() {
    ...
    if (formData[fieldCode].length && formData[fieldName].length > 0) {
      try {
        ...
        if (response.status === 201) {
          ...
          setReusableSnackBar((prev) => ({
            severity: "success",
            message: `${SelectedMasterDatatab} create Sucessfully!`,
            open: true,
          }));
        }
      } catch (e: any) {
      ...
      <div className="create-plant-wrapper-div">
        <div className="create-plant-field-place-div">
          ...
          <div className="create-plant-wrapper-single-input">
            ...
            <OutlineTextField
              fullWidth
              placeholder={`Enter ${SelectedMasterDatatab} Unit`}
              type="text"
              value={formData ? formData[fieldCode] : ""}
              onChange={handleInputChange}
              helperText={
                plantFormError.code
                  ? `${SelectedMasterDatatab}Unit Should not be empty`
                  : ""
              }
