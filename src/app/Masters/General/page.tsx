...
import CreateGeneral from "./(GeneralAction)/_CreateGeneral";
import EditGeneral from "./(GeneralAction)/_EditGeneral";
import Generalgrid from "./(GeneralAction)/_GeneralGrid";
import "./style.scss";
...
export default function General() {
  ...
  useEffect(() => {
    if (setSelectedMasterDatatab) {
      setSelectedMasterDatatab("IndustrySector");
    }
  }, [setSelectedMasterDatatab]);
  const auth = UseAuth();
  if (!auth) {
  ...
  const tabRenderValuePlant: Record<string, ReactNode> = {
    ...
    edit: <EditGeneral EditDataGet={EditDataGet} />,
  };
